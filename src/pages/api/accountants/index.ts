...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getAccountants() {
    const data = await prisma.accountant
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'accountant'));
  ...
  async function createAccountant() {
    ...
    const data = await prisma.accountant.create({
      data: body,
    });
    return res.status(200).json(data);
  }
